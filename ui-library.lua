...
function library.new(theme)
	...
	function pages:Toggle()
		...
		local dir = toggled and -0.1 or 0.1
		local tweeninfo = TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
		local ToggleTween = TweenService:Create(UIFrame, tweeninfo, {Position=UDim2.new(dir,0,0.5,0)})
		ToggleTween:Play()
	...
	function pages:AddPage(PageTitle)
		...
		function sections:AddSection()
			...
			function elements:NewDropdown(DropdownTitle, callback, Options)
				...
				DropdownButton.MouseButton1Click:Connect(function()
					local tog = not DropdownContainer.Visible
					DropTween(tog)
				end)
		...
		local function ChangeZoom(min, max)
		...
		UIFrame.MouseEnter:Connect(function()
			...
			ChangeZoom(currentzoom, currentzoom)
			MouseScroll['InFrame'] = true
		end)

		UIFrame.MouseLeave:Connect(function()
			ChangeZoom(DefaultZoom['Min'], DefaultZoom['Max'])
			MouseScroll['InFrame'] = false
		end)
